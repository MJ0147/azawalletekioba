...
2. **GET `/api/wallet/dashboard`** (JSON Response)
   ...
   - Includes the configured TON merchant wallet
...
- Features:
  - Wallet information display (TON address)
...
2. **Backend Forecast Service** (agent.py)
   ...
   - Measures Supabase round-trip latency for the Supabase Health card
...
IDIA_TON_JETTON_ADDRESS=<idia-jetton-master-address>
...
# Supabase Health card (probes /auth/v1/health)
...
✅ **Wallet Integration**: Displays the configured TON wallet
